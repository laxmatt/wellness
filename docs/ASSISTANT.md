...
The proposal now carries `matchesByKey`, what each hard constraint admits on its own, computed in the route by the same `evaluateCondition` that decided `matching`, so the placeholder-price treatment comes with it rather than being restated. Hard constraints are ANDed, so what remains after any number of removals is the intersection of the sets still standing. That is what makes a second removal correct: a set precomputed for removing one constraint is wrong the moment a second goes, and this composes instead. The labels come from the same breakdown, so the band names the exact remaining constraints in the site's own words.
