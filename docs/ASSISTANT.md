...
**A figure a source states two ways is not a fact.** Hooga's HG300 page says "over 73 mW/cm2" in its highlights and "73" in its specification table. Recorded as a bound, the site answered "more than 73" and "not 73" with a page that says both; recorded as exact, it would have answered "73" with a page that says more. The record carries `disputed` now: the value stays visible as "73 mW/cm², disputed", both statements stay in its note, and it is withheld from matching, from scoring and from the comparison ordering. It is not "not stated", which would be a different and false claim about the source. The score breakdown says "Withheld, the source states it two ways" rather than "Not stated".

**A prototype amount is not a cheaper offer.** The shown price was the lowest offer of any kind, so HG300's real $199 lost to a prototype $149 on an Amazon record and the page said "Check current price" about a price it had. The shown price is now the lowest offer whose amount is real, with a prototype one standing in only when every offer carries one. The retailer count, the price provenance, the affordability input and the comparison's "from $X" all read that same set.
