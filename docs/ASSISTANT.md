...
**Placeholder values were matchable all along.** `specs` withheld demo values from every screen and from the assistant, `attributes` kept them, and `evaluateCondition` reads `attributes`. OLIPOP's demo caffeine zero qualified a search for zero caffeine for as long as it has existed, as did every other demo value in the catalogue. `attributes` now holds only values that can be used as fact. The score still names what it could not use, read from provenance rather than from the attributes it no longer contains.
