...
**A direction has to mean something on the key it points at.** `toEngineConstraints` accepted any non-money preference. Cold plunge's `tub_type` runs barrel, tub and inflatable with no ranks, so `prefer_low` on it orders nothing; the site printed "Ranking for lower tub_type" and quietly scored an equality match instead. A direction now needs an ordinal basis, price, a number, or an enum whose options carry ranks, and `prefer_value` needs a value the category actually has. Anything else fails visibly rather than being interpreted.
