...
**Product facts are rendered by this site, not written by the model.** Each product card carries the value and who says so, taken from the catalogue: "verified by this site", "reported by the maker", or "source not recorded". A placeholder price is no longer sent to the model at all. It used to go with a label saying not to rely on it, and the model quoted it to a shopper as "$139"; a label is advice, withholding is not.

**Prose is verified against that data before it is shown.** `src/domain/reply-verification.ts` refuses two things it can decide from the approved data: a figure the data does not hold, and a sentence explaining what a product does to a body. Either replaces the prose with a limitation saying what the site can answer instead. This is not a general truth check and does not claim to be: it is two specific refusals over a defined risk class, and it fails towards saying less.
