...
describe("a record may not claim the maker while citing a retailer", () => {
  ...
  // This carried an exception list while four Cold Pod attributes still claimed
  // the maker over an Amazon URL. They now say "retailer", so the rule holds
  // outright and the list is gone.
  it("holds for every attribute in the catalogue", () => {
    const found: string[] = [];
