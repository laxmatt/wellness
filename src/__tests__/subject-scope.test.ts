...
describe("subjects this site has no catalogue for", () => {
  it("names the listed subject", () => {
    ...
    expect(on(coldPlunge, "looking for a massage gun")).toEqual({ kind: "absent_subject", labels: ["massage guns"] });
  ...
  it("survives an ordinary misspelling", () => {
    ...
    expect(on(redLight, "any massage gun's?")).toEqual({ kind: "absent_subject", labels: ["massage guns"] });
  });
