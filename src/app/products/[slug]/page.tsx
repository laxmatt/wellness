      ...
      <Container className="pt-4">
        ...
        <div className="mt-4 grid gap-8 lg:grid-cols-12 lg:gap-12">
          ...
          <div className="flex flex-col gap-5 lg:col-span-6">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {view.flags.demo ? <span className="rounded-pill border border-ember px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.08em] text-ember-deep">Demo data</span> : null}
              {view.flags.newArrival ? <span className="rounded-pill bg-honey-soft px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.08em] text-ink">New</span> : null}
            </div>
            <div>
              <Link href={`/brands/${view.brand.slug}`} className="eyebrow hover:text-ink">
                {view.brand.name}
              </Link>
              <h1 className="font-display mt-1 text-4xl leading-[0.98] sm:text-5xl">{view.name}</h1>
              {alsoValue ? <p className="mt-2 text-sm font-semibold text-moss">Also the strongest value in {cat.name.toLowerCase()}.</p> : null}
              {item.badgeReasons[0] ? <p className="mt-1 text-sm text-ink-soft">{item.badgeReasons[0]}</p> : null}
            </div>
            <p className="text-base text-ink-soft">{view.description}</p>
            <PriceDisplay price={view.price} size="lg" />
            <div className="divide-y divide-line rounded-card border border-line bg-paper px-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {lowest ? (
                <a href={view.offers.length > 1 ? "#retailers" : lowest.url} target={view.offers.length > 1 ? undefined : "_blank"} rel={view.offers.length > 1 ? undefined : "sponsored nofollow noopener"} className="tap inline-flex h-13 items-center justify-center rounded-pill bg-ink px-6 text-base font-semibold text-paper hover:bg-ember-deep">
            ...
            {primaryStrength(view, cat) ? (
              <p className="text-sm">
                <span className="font-semibold text-moss">Why it ranks: </span>
                {primaryStrength(view, cat)}
              </p>
            ) : null}
          </div>
        ...
        <div className="mt-14 flex flex-col gap-12">
          <section>
            ...
            <p className="mt-1 text-sm text-ink-mute">Derived from the specs below by published rules. No experiential claims without a source.</p>
            <div className="mt-5">
          ...
          <section id="retailers">
            ...
            <p className="mt-1 text-sm text-ink-mute">Prices shown as last checked. Retailer relationships never affect ranking.</p>
            <div className="mt-5">
              <OfferList view={view} />
            </div>
          </section>
          ...
          {similar.length > 0 ? (
            <section>
              <div className="flex items-end justify-between gap-4">
                ...
                <Link href={`/${cat.slug}`} className="tap hidden items-center text-sm font-semibold text-ember-deep hover:underline sm:inline-flex">
