...
type Item = { image: ImageAsset; name: string; brand: string; url: string; affiliateStatus: AffiliateStatus };
        ...
        <div className={styles.track} data-paused={paused}>
          {[0, 1].map((copy) => (
            <div className={styles.group} key={copy} aria-hidden={copy === 1 ? true : undefined}>
              {items.map(({ image, name, brand, url, affiliateStatus }) => (
                <a className={styles.card} key={image.id} href={url} {...outboundLinkProps(affiliateStatus)} tabIndex={copy === 1 ? -1 : undefined} aria-label={`${name} — visit retailer (opens in a new tab)`}>
