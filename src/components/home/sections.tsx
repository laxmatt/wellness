      ...
      <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 pb-10 pt-8 sm:px-6 lg:grid-cols-12 lg:items-end lg:gap-12 lg:px-8 lg:pb-16 lg:pt-14">
        <div className="lg:col-span-5">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <Link href="/red-light" className="tap inline-flex items-center rounded-pill bg-ink px-6 text-base font-semibold text-paper hover:bg-ember-deep">
              Shop red light
            </Link>
            <Link href="/explore" className="tap inline-flex items-center rounded-pill border border-line-strong bg-paper px-6 text-base font-semibold hover:border-ink">
              Explore all
            </Link>
          </div>
        </div>
        <div className="lg:col-span-7">
          <div className="relative overflow-hidden rounded-card shadow-float">
            ...
            <div className="absolute bottom-4 left-4 rounded-card bg-paper/90 px-4 py-3 backdrop-blur">
              <p className="eyebrow">This week</p>
              <p className="font-display text-lg">Full-body panels under $1,000</p>
              <Link href="/red-light/under-1000" className="text-sm font-semibold text-ember-deep hover:underline">
                See the picks
              </Link>
            </div>
          </div>
        ...
        {pages.map((p) => {
          ...
          return (
            <Link key={p.cat.id} href={`/${p.cat.slug}`} className="lift group relative overflow-hidden rounded-card shadow-card hover:-translate-y-0.5 hover:shadow-float">
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink/70 via-ink/20 to-transparent p-5 text-paper">
                <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-paper/80">{p.products.length} products</p>
              ...
              <span className="absolute right-3 top-3 rounded-pill bg-paper/85 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.08em] text-ember-deep">Demo image</span>
            </Link>
          );
        })}
      </div>
...
export function SectionHeading({ eyebrow, title, href, linkLabel }: { eyebrow: string; title: string; href?: string; linkLabel?: string }) {
  return (
    <div className="flex items-end justify-between gap-4">
      ...
      {href ? (
        <Link href={href} className="tap hidden items-center text-sm font-semibold text-ember-deep hover:underline sm:inline-flex">
          {linkLabel ?? "See all"}
        </Link>
      ) : null}
    </div>
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        {pages.map((p) => (
          <div key={p.cat.id} className="rounded-card border border-line bg-paper p-5">
            <p className="eyebrow">{p.cat.navLabel}</p>
            <ul className="mt-3 flex flex-wrap gap-2">
              {p.cat.facets.map((f) => (
                <li key={f.slug}>
                  <Link href={`/${p.cat.slug}/${f.slug}`} className="tap inline-flex items-center rounded-pill border border-line-strong bg-paper px-3.5 text-sm font-medium hover:border-ink">
                    {f.label}
                  </Link>
                </li>
              ))}
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <ul className="mt-6 flex flex-wrap gap-2">
        {brands.map((b) => (
          <li key={b.id}>
            <Link href={`/brands/${b.slug}`} className="tap inline-flex items-center rounded-pill bg-paper px-4 text-sm font-semibold shadow-card hover:bg-ivory-deep">
              {b.name}
            </Link>
          </li>
        ))}
...
export function HowWeChooseCallout() {
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="rounded-card bg-ink p-6 text-paper sm:p-10">
        <div className="grid gap-8 lg:grid-cols-12 lg:items-start">
          <div className="lg:col-span-4">
            <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-paper/60">How we choose</p>
            <h2 className="font-display mt-2 text-3xl sm:text-4xl">Opinionated where the facts allow it. Neutral everywhere else.</h2>
            <Link href="/how-we-choose" className="tap mt-5 inline-flex items-center rounded-pill bg-paper px-5 text-sm font-semibold text-ink hover:bg-ivory-deep">
              Read the rules
            </Link>
          </div>
          <dl className="grid gap-6 sm:grid-cols-3 lg:col-span-8">
            {points.map(([t, d]) => (
              <div key={t}>
                ...
                <dd className="mt-1 text-sm text-paper/75">{d}</dd>
              </div>
            ))}
          </dl>
        </div>
