...
type Session = { id: string; touched: number; source: JourneyEvent['source']; test: boolean; sequence: number };
...
export function sendJourney(event: JourneyEvent['event'], page: JourneyEvent['page'], item?: { product_id?: string; product_name?: string; retailer_name?: string }) {
  ...
  try {
    ...
    const test = session?.test === true || localStorage.getItem(TEST_KEY) === '1' || location.hostname === 'localhost' || location.hostname === '127.0.0.1' || params.get('utm_content') === 'tracking_test';
    if (!session || now - session.touched > 30 * 60 * 1000 || session.test !== test) {
      ...
      session = { id: crypto.randomUUID(), touched: now, source, test, sequence: 0 };
    }
    session.touched = now;
    ...
    void fetch('/api/traffic', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: crypto.randomUUID(), session: session.id, event, page, sequence: session.sequence, source: session.source, test: session.test, ...details }),
