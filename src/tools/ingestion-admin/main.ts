...
function exclusionSection(): HTMLElement {
  ...
  for (const [i, rule] of profile.exclusions.entries()) {
    ...
    row.append(field("Test", select(rule.op, [{ value: "not_equals", label: "is not exactly" }, { value: "equals", label: "is exactly" }, { value: "contains", label: "contains" }, { value: "not_contains", label: "does not contain" }, { value: "contains_word", label: "contains whole word(s)" }, { value: "not_contains_word", label: "does not contain whole word(s)" }, { value: "not_one_of", label: "is not one of (separate with ||)" }, { value: "empty", label: "is empty" }, { value: "not_empty", label: "is not empty" }, { value: "starts_with", label: "starts with" }, { value: "not_starts_with", label: "does not start with" }], (v) => { rule.op = v as ExclusionRule["op"]; state.preflight = null; render(); })));
